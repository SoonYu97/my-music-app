...
import LyricDisplay2 from "./LyricDisplay2";
...
const VideoLyrics: React.FC<{ videoSrc: string; lyricsSrc: string }> = ({
  ...
  const handleTimeUpdate = (event: React.SyntheticEvent<HTMLVideoElement>) => {
    ...
    if (metadata?.lyrics) {
      const index = metadata.lyrics.findIndex(
        (lyric) => lyric.time > currentTime
      );
      setCurrentLyricIndex(index === -1 ? metadata.lyrics.length - 1 : index);
    }
  };
  ...
  const handleScrollPause = () => {
    ...
    }, 3000); // Pause scrolling for 3 seconds after manual scroll
  };
    ...
    <div className="flex flex-col h-screen mx-auto">
      {metadata && (
        <div className="p-4 text-center">
          ...
          <h4 className="text-lg">{videoSrc}</h4>
        </div>
      )}
